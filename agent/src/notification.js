...
class Notification {
  ...
  send(level, type, message, extra = {}) {
    ...
    if (this.webhookUrl && (this.channel === 'webhook' || level === 'error' || level === 'warning')) {
  ...
  _printToTerminal(entry) {
    ...
    const reset = '\x1b[0m';
    console.log('  ' + (colors[entry.level] || '') + '[' + entry.level.toUpperCase() + ']' + reset + ' [' + entry.type + '] ' + entry.message);
  }

  _sendWebhook(entry) {
    if (!this.webhookUrl) return;
    try {
      const color = entry.level === 'error' ? '0xf85149' : entry.level === 'warning' ? '0xd29922' : entry.level === 'success' ? '0x3fb950' : '0x58a6ff';
  ...
  getSummary() {
    ...
    for (const entry of history) {
      ...
      byType[key].count++;
      byType[key].levels[lvl] = (byType[key].levels[lvl] || 0) + 1;
    }
